...
class Db():
    ...
    def add_room(self, password_hash, game_type, game_content):
        try:
            room_id = secrets.token_urlsafe(6)
            game_content = json.dumps(game_content)
            self.conn.execute('INSERT INTO rooms (room_id, password_hash, game_type, game_content) VALUES (?, ?, ?, ?)', (room_id, password_hash, game_type, game_content, ))
            return room_id
    ...
    def add_user(self, room_id):
        try:
            self.conn.execute('UPDATE rooms SET users_joined = users_joined + 1 WHERE room_id = ?', (room_id, ))
        except Exception as error:
            print(f"Database error: {error}")
    ...
    def set_game_content(self, room_id, game_content):
        try:
            game_content = json.dumps(game_content)
